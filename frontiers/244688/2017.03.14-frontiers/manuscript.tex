...
that take place within the thin layer $\ell$.
{\color{red}
As a test case, we calculate $\boldsymbol{\chi}(-2\omega;\omega,\omega)$ for the
Si(111)(1$\times$1):H surface, and present a layer-by-layer study of the
susceptibility to elucidate the depth dependence of the SHG spectrum. We then
use the depth dependent three-layer model to calculate the SSHG yield, and
contrast the calculated spectra with experimental data. We produce improved
results over previous published work, as this treatment can reproduce key
spectral features, is computationally viable for many systems, and most
importantly remains completely \emph{ab initio}.
}
