...
\change{compares favorably} % ref1.13
...
\onlinecite{mejiaPRB02}, where the \correction{3-layer} model was introduced for
...
$T^{\alpha\beta}_{i}(\omega) = t^{\alpha\beta}_{i}(2\omega)$,} % ref2.04
...
\frac{\pi|e|^3}{2\hbar^2}\int\frac{\correction{d^{3}k}} % ref2.05
...
and are calculated in $\text{pm}^{2}/\text{V}$.} % ref1.09
...
result.\cite{hoganPRB03, castilloPRB03, nicolasPRB15}} % ref1.09
...
\correction{relaxed coordinates have a peak position} % ref1.02
...
the effects of temperature. We can also see from Ref. \onlinecite{hoferAPA96}
...
Ref. \onlinecite{hoferAPA96}. 
\correction{Theoretical curves are broadened with $\sigma=0.075\,\text{eV}$.
} % ref1.08
\correction{Experimental data was taken at 80 K.} % ref1.11
\label{fig:Xxxx}}
\end{figure}
...
conducted under very low temperature conditions.} % ref1.13
...
\change{we see that all three models reproduce the lineshape} % ref1.13
...
\onlinecite{mejiaPRB02}\correction{, measured at room temperature.} % ref1.11
\label{fig:RpS}}
\end{figure}
...
Experimental data taken from Ref.
\onlinecite{mitchellSS01}\correction{, measured at room temperature.} % ref1.11
...
closest to the experiment.} % ref1.04
...
favorably with the experimental data.} % ref1.05
...
\correction{All curves are broadened with $\sigma=0.075\,\text{eV}$.} % ref1.08 
...
experiment, but are 40 times more intense.} % ref1.13
...
\correction{but with eight times} % ref1.10
...
closest in intensity to the experiment.} % ref1.04
\correction{Additionally, Ref. \onlinecite{dadapPRB97} shows that low temperature
...
\ref{fig:mitchellRpP}, and towards our theoretical results.} % ref1.03
...
\onlinecite{mitchellSS01}. The \correction{3-layer} model is, as before,
\change{close to the experiment in} % ref1.13
both peak position and intensity. Intensity 
\change{is almost the same the experimental value.} % ref1.13
...
our calculation to $T=0$ K.} % ref1.01
...
closer to the experiment for all cases of $\mathcal{R}_{\mathrm{iF}}$.} % ref1.13
...
\correction{3-layer} and \correction{2-layer-vacuum} models.} % ref1.13
