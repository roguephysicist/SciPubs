...
buried interfaces. However, SSHG is compatible with non-UHV conditions and 
...
implemented. 
In the works that include this operator,  the velocity gauge was
...
LDA superscript. Any other approximation can be used (like the
generalized gradient approximation) and our 
derivation remains the same. Then,
\begin{equation*}
H^{\mathrm{LDA}}_{0}(\mathbf{r},\mathbf{p})
=\frac{p^{2}}{2m_e}+V(\mathbf{r},\mathbf{p})
\end{equation*}
with $m_e$ the mass of the electron, 
$\mathbf{p}=-i\hbar\boldsymbol{\nabla}$ 
its canonical momentum, and 
$V$
...
According to Ref. ~\onlinecite{ismailPRL01}, we first
...
3D Brillouin Zone (BZ) integrals  
where special care was taken to examine the double resonances
of Eq.~\eqref{chis}. \cite{nastosPRB05}
Note that the Brillouin zone for the slab geometry collapses to a               
2D-zone, with only one $\mathbf{k}$-point along the $z$-axis.
...
shifts the spectra from its LDA value to higher energies as expected.
 However, contrary 
...
The reduction of the spectral strength is in 
